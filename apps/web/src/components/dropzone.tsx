...
import { type UseTrackUploadReturn } from "@/hooks/use-track-upload";
...
type DropzoneProps = UseTrackUploadReturn & {
  className?: string;
};
  ...
  return (
    <DropzoneContext.Provider value={{ ...restProps }}>
      <div
        {...getRootProps({
          className: cn(
            "rounded-lg border-2 border-border p-6 text-center text-foreground transition-colors duration-300",
...
const DropzoneEmptyState = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-y-2", className)}>
      ...
      <p className="text-sm">
        Upload{maxFiles && maxFiles > 1 ? ` ${maxFiles}` : ""} audio file
